...
//Price in $ per GHzsec of computation
...
func RunFunction(id int, runtime uint64, memory uint64) {
	...
	atomic.AddUint64(&TotalComputeCPU, uint64(computeCPU))
...
func CalculatePrice() string {
	...
	//Subtract free CPU time from total CPU time (convert CPU time back to GHzsec by dividing by 1000000)
	totalComputeCPU := float64(TotalComputeCPU)/1000000 - freeComputeCPU
	//Multiply remainder by computeCPU price to get cost
	compCPUPrice := totalComputeCPU * COMPUTE_CPU_PRICE
...
func CalculateFnPrice(numfn int) []float64 {
	...
	for i := 0; i < numfn; i++ {
		...
		totalComputeCPU := float64(functions[i][3])/1000000 - freeComputeCPU
		compCPUPrice := totalComputeCPU * COMPUTE_CPU_PRICE
